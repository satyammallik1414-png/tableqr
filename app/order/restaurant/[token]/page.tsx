...
export default async function RestaurantQRPage({ params }: PageProps) {
  ...
  if (!session?.user) redirect(`/customer/register?callbackUrl=${encodeURIComponent(`/order/restaurant/${token}`)}`);
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 p-6 text-center text-slate-800 font-sans">
        <div className="max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <h2 className="text-xl font-bold text-navy-900">QR Code Unavailable</h2>
          <p className="mt-2 text-sm text-slate-600">
            {result.error || "This QR code could not be verified or is no longer active."}
          </p>
          <div className="mt-6 flex flex-col gap-2">
            <a href={`/order/restaurant/${token}`}>
  ...
  const { qr, restaurant, branch, availableTables, categories, paymentSettings, taxSettings } = result.data;
